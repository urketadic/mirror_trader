...
'''
def init_copy_clients(mainusdt,mainlink):
	global copy_clients
	...
	for cr in creds:
		...
		print("Main usdt: " +str(mainusdt) )
		print("Mainlink: " +str(mainlink) )		
		...
		try:
			...
			clientusdt = float(newClient.get_asset_balance(asset='USDT')['free'])
			clientlink = float(newClient.get_asset_balance(asset='LINK')['free'])
			print('Client usdt: '+ str(clientusdt) )
			print('Client link: '+ str(clientlink) )
			...
			copy_clients[-1].BlncRateLink = float(clientlink)/mainlink
			copy_clients[-1].BlncRateUSDT = float(clientusdt)/mainusdt
			copy_clients[-1].accName = accName
			copy_clients[-1].active = True
			print('Client-to-main ratio balance Link: '+ str(copy_clients[-1].BlncRateLink))
			print('Client-to-main ratio balance USDT: '+ str(copy_clients[-1].BlncRateUSDT))
...
##			bMessage = bMessage + clientMsg
		...
		if orderData['o']=='MARKET' and orderData['X']=='NEW':
			...
			tIFrc = orderData['f']
			ordQty = 2
			#ordQty = float(orderData['q'])*round(client.BlncRateUSDT, 3)
			...
			client.create_order( symbol=symb , side=ordSide, type=ORDER_TYPE_MARKET, quantity=ordQty)
			bMessage = ''
			if count==1:
